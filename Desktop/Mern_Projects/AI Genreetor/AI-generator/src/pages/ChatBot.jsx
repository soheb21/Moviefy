...
const ChatBot = () => {
  ...
  const handler = async () => {
    try {
      const { data } = await axios.post("http://localhost:8000/api/v1/openai/chatbot", { text }, {
        headers: {
      ...
      <div className="flex flex-col h-full w-full gap-1 p-4">
        <div className="w-full ">
          <div className="flex justify-between px-2 my-2">
            <label htmlFor="message" className="block mb-2 text-sm font-medium  text-white ">ChatBot</label>
        ...
        <div className="h-full w-full ">
          <label htmlFor="message" className="block mb-2 text-sm font-medium  text-center text-white ">ChatBot</label>
          {chatBot ? (
